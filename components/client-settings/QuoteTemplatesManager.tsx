...
export function QuoteTemplatesManager({ clientId }: { clientId: string }) {
  ...
  if (loading) return <p className="text-sm text-ink-tertiary">Loading templates…</p>;

  return (
    <section className="space-y-3">
      <div>
        <h3 className="font-display text-lg">Quote templates</h3>
      ...
      {templates.length === 0 ? (
        <p className="rounded-xl border border-dashed border-border px-4 py-8 text-center text-[13px] text-ink-tertiary">
      ...
      {toast ? <p className="text-[12px] text-[var(--accent)]">{toast}</p> : null}
    </section>
  );
}
...
function TemplateEditorRow({
  ...
  return (
    <li className="rounded-xl border border-border">
      ...
      >
        <span className="text-[14px] font-semibold text-ink-primary">{template.name}</span>
      ...
      {expanded ? (
        <div className="space-y-3 border-t border-border px-4 py-4">
          ...
          <textarea
            className="textarea-base min-h-[4rem]"
          ...
          <div className="space-y-2">
            {items.map((it) => (
              <div key={it.key} className="grid grid-cols-12 gap-2">
                <input
                  className="input-base col-span-4 h-8 text-[12px]"
                  placeholder="Item"
                  value={it.item_name}
                  onChange={(e) =>
                    setItems((rows) =>
                ...
                <button
                  type="button"
                  className="col-span-1 text-ink-tertiary hover:text-[var(--danger)]"
            ...
            <button type="button" onClick={addRow} className="text-[12px] text-ink-secondary hover:text-ink-primary">
              + Add line item
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={onRemove} className="btn-ghost text-sm text-[var(--danger-fg)]">
              Remove
            </button>
          </div>
        </div>
