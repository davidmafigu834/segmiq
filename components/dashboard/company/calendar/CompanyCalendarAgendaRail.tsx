  ...
  return (
    <section className="border-b border-sales-border-subtle p-4" data-course-target="calendar-date-navigation">
      <div className="flex items-center justify-between">
        <h2 className="text-[13px] font-semibold text-sales-text-primary">{format(monthStart, "MMMM yyyy")}</h2>
    ...
    <>
      <section className="border-b border-sales-border-subtle p-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h2 className="text-[12px] font-semibold text-sales-text-primary">{format(selectedDate, "EEE, MMM d, yyyy")}</h2>
            <p className="mt-0.5 text-[10px] text-sales-text-muted">Selected day</p>
          </div>
          <span className="text-[10px] font-medium text-sales-text-muted">{selectedDayEvents.length} {selectedDayEvents.length === 1 ? "activity" : "activities"}</span>
        </div>
        {selectedDayEvents.length ? (
          ...
          <div className="flex min-h-[118px] flex-col items-center justify-center text-center">
            ...
            <p className="mt-2 text-[11px] font-medium text-sales-text-primary">No activities scheduled for this day.</p>
            {canCreate ? <button type="button" onClick={onAdd} className="mt-2 inline-flex items-center gap-1 text-[11px] font-semibold text-sales-brand-fg hover:underline"><Plus size={12} /> New Activity</button> : null}
          </div>
        )}
      </section>
      <section className="p-4" data-course-target="calendar-upcoming">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <h2 className="text-[12px] font-semibold text-sales-text-primary">Upcoming</h2>
            {upcomingEvents.length ? <span className="rounded-full bg-sales-surface-subtle px-1.5 py-0.5 text-[9px] text-sales-text-muted">{upcomingEvents.length}</span> : null}
          </div>
          <button type="button" onClick={onViewAll} className="text-[10px] font-semibold text-sales-brand-fg hover:underline">View all</button>
        </div>
        ...
        ) : <p className="py-8 text-center text-[11px] text-sales-text-muted">Nothing else scheduled.</p>}
      </section>
    </>
  );
}
...
function EventDetail({
  ...
  return (
    <div className="p-4">
      <button type="button" onClick={onBack} className="inline-flex items-center gap-1 text-[11px] font-semibold text-sales-text-secondary hover:text-sales-text-primary"><ArrowLeft size={13} /> Back to agenda</button>
      <div className="mt-4 flex items-start gap-3 border-b border-sales-border-subtle pb-4">
        ...
        <div className="min-w-0">
          <h2 className="text-[15px] font-semibold leading-snug text-sales-text-primary">{event.title}</h2>
          <div className="mt-1.5 flex flex-wrap gap-1.5"><span className="rounded-[5px] border border-sales-border px-1.5 py-0.5 text-[9px] font-medium text-sales-text-secondary">{meta.label}</span><span className={cn("rounded-[5px] px-1.5 py-0.5 text-[9px] font-medium", statusClass(event))}>{statusLabel(event)}</span></div>
        </div>
      </div>
      <div className="divide-y divide-sales-border-subtle">
        <div className="flex gap-3 py-3.5"><CalendarDays size={15} className="mt-0.5 shrink-0 text-sales-text-muted" /><div><p className="text-[10px] font-medium uppercase tracking-[0.04em] text-sales-text-muted">Date and time</p><p className="mt-1 text-[11px] text-sales-text-primary">{formatCalendarEventRange(event, timezone)}</p><p className="mt-0.5 text-[9px] text-sales-text-muted">{timezone}</p></div></div>
        <div className="py-3.5"><p className="text-[10px] font-medium uppercase tracking-[0.04em] text-sales-text-muted">Related {relationLabel(event)}</p><Link href={event.relatedHref} className="mt-2 flex items-center justify-between rounded-[9px] border border-sales-border bg-sales-surface-subtle px-3 py-2.5 hover:border-sales-border-strong"><span className="min-w-0"><span className="block truncate text-[11px] font-semibold text-sales-text-primary">{event.relatedLabel}</span>{event.relatedSecondary ? <span className="mt-0.5 block truncate text-[10px] text-sales-text-muted">{event.relatedSecondary}</span> : null}</span><ExternalLink size={13} className="shrink-0 text-sales-text-muted" /></Link></div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        {event.phone ? <a href={`tel:${event.phone}`} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-[9px] border border-sales-border text-[11px] font-semibold text-sales-text-primary hover:bg-sales-surface-hover"><Phone size={13} /> Call</a> : null}
        {event.leadId ? <Link href={`/client/inbox?lead=${encodeURIComponent(event.leadId)}`} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-[9px] border border-sales-border text-[11px] font-semibold text-sales-text-primary hover:bg-sales-surface-hover"><SiWhatsapp size={13} color="#25D366" /> WhatsApp</Link> : null}
        <Link href={event.relatedHref} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-[9px] border border-sales-border text-[11px] font-semibold text-sales-text-primary hover:bg-sales-surface-hover"><ExternalLink size={13} /> Open {relationLabel(event)}</Link>
        {event.canComplete ? <button type="button" disabled={busy} onClick={onComplete} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-[9px] border border-sales-border text-[11px] font-semibold text-sales-text-primary hover:bg-sales-surface-hover disabled:opacity-50"><Check size={13} /> Mark complete</button> : null}
      </div>
...
export function CompanyCalendarAgendaRail({
  ...
  return (
    <aside className="h-full min-h-[760px] overflow-hidden rounded-[13px] border border-sales-border bg-sales-surface shadow-sales-card" data-course-target="calendar-right-agenda">
      {selectedEvent ? (
