  ...
  return (
    <>
      <button
        ...
        className="hidden sm:flex items-center gap-2 px-3 py-1.5 text-[13px] text-[#666] dark:text-white/55 hover:text-black dark:hover:text-white transition-colors"
      >
        <Search className="w-3.5 h-3.5" />
        <span>Search</span>
        <kbd className="hidden md:inline text-[10px] bg-black/[0.04] dark:bg-white/[0.08] px-1.5 py-0.5 font-mono">⌘K</kbd>
      </button>
      <button
        type="button"
        onClick={() => setOpen(true)}
      ...
      {open && (
        <div className="fixed inset-0 z-[100]">
          ...
          <div className="relative mx-auto mt-[12vh] max-w-[560px] px-4">
            <div className="bg-white dark:bg-[#141414] border border-black/10 dark:border-white/10 shadow-lg overflow-hidden">
              ...
              <div className="max-h-[360px] overflow-y-auto">
                ...
                {results.map((p) => (
                  <Link
                    ...
                    onClick={() => setOpen(false)}
                    className="block px-4 py-3.5 border-b border-black/[0.06] dark:border-white/[0.06] last:border-0 hover:bg-black/[0.02] dark:hover:bg-white/[0.04] transition-colors"
                  >
                    <div className="text-[10px] font-bold uppercase tracking-wider text-[#76B900] dark:text-[#D4FF4F]">{p.categoryLabel}</div>
