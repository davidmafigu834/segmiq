...
export function TodayAgenda({
  ...
  return (
    <div className="cal-card overflow-hidden">
      <div className="flex items-center justify-between gap-2 border-b border-[#EAECF0] px-3.5 py-3">
        <h2 className="text-[14px] font-semibold text-[#101828]">Today&apos;s agenda</h2>
        <p className="shrink-0 text-[12px] font-medium text-[#667085]">{header}</p>
      </div>

      {!events.length ? (
        <div className="flex flex-col items-center justify-center px-3 py-8 text-center">
          <CalendarDays size={18} strokeWidth={1.8} className="text-[#98A2B3]" aria-hidden />
          <p className="mt-2 text-[13px] font-semibold text-[#101828]">No events scheduled</p>
          <p className="mt-0.5 text-[12px] text-[#667085]">Your day is clear.</p>
          <button
            type="button"
            onClick={onAddEvent}
            className="mt-3 inline-flex h-9 items-center gap-1.5 rounded-[9px] bg-[#D4FF4F] px-3 text-[12px] font-semibold text-[#101828]"
          >
            ...
            {events.map((event) => {
              ...
              return (
                <li key={event.id}>
                  ...
                  >
                    <span className="text-[12px] font-semibold tabular-nums leading-none text-[#667085]">
                    ...
                    <span className="flex min-w-0 items-start gap-2">
                      ...
                      <span className="min-w-0">
                        <span className="flex min-w-0 items-center gap-1.5">
                          <span className="truncate text-[13px] font-semibold leading-tight text-[#101828]">
                            {eventTitle(event)}
                          </span>
                          {isNext ? (
                            <span className="shrink-0 rounded-[4px] bg-[rgba(212,255,79,0.4)] px-1 py-px text-[9px] font-bold uppercase tracking-wide text-[#101828]">
                              Next
                            </span>
                          ) : null}
                        </span>
                        {event.customerName ? (
                          <span className="mt-0.5 block truncate text-[12px] leading-tight text-[#667085]">
                            {event.customerName}
                          </span>
                        ) : null}
                        {event.overdue ? (
                          <span className="mt-0.5 block text-[11px] font-medium leading-tight text-[#B42318]">
                            Overdue
                          </span>
                        ) : null}
                      </span>
