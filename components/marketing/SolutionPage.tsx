    ...
    <>
      ...
      <section className="pt-16 pb-12">
        <div className="mx-auto max-w-[1100px] px-5 grid lg:grid-cols-2 gap-10 items-center">
          ...
          <div className={`group relative h-[360px] rounded-2xl ring-1 ${m.ring} overflow-hidden`}>
      ...
      <section className={`py-12 ${m.sectionBand}`}>
        <div className="mx-auto max-w-[1100px] px-5">
          <div className="max-w-[680px]">
            <div className={m.kicker}>THE PROBLEM</div>
            <h2 className="text-[28px] font-extrabold leading-tight mt-2">{d.problemH2}</h2>
            <p className={`mt-3 text-[15px] ${m.muted}`}>{d.problemP}</p>
          </div>
        </div>
      </section>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-9">
            {d.steps.map(({ Icon, t, d: desc }) => (
              <div key={t} className={`${m.cardHover} p-5`}>
                ...
                <p className={`text-sm ${m.muted} mt-1`}>{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-14">
        <div className="mx-auto max-w-[1100px] px-5 grid lg:grid-cols-2 gap-10 items-center">
          <div className={`relative h-[320px] rounded-2xl ring-1 ${m.ring} overflow-hidden order-2 lg:order-1`}>
            <Image src={d.cloudImg} alt="Finished work" fill sizes="(max-width:1024px) 100vw, 50vw" className="object-cover" />
          </div>
          <div className="order-1 lg:order-2">
            <div className={m.kicker}>WITH SEGMIQ CLOUD</div>
            <h2 className="text-[28px] font-extrabold leading-tight mt-2">{d.cloudH2}</h2>
            <p className={`mt-3 text-[15px] ${m.muted}`}>{d.cloudP}</p>
      ...
      <section className="py-10">
        <div className="mx-auto max-w-[1100px] px-5">
          <div className={`flex flex-wrap items-center justify-center gap-2 text-sm font-medium ${m.muted}`}>
            <span className={`${m.faint} mr-1`}>Also for:</span>
            {d.others.map((o) => (
              <a key={o.label} href={o.href} className={`rounded-full border ${m.border} px-4 py-1.5 hover:border-white/30 hover:text-white`}>{o.label}</a>
            ))}
          </div>
        </div>
      </section>
