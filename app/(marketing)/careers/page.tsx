    ...
    <>
      ...
      <section className="pt-16 pb-12">
        <div className="mx-auto max-w-[1100px] px-5 max-w-[760px]">
          <div className={m.kicker}>CAREERS</div>
          ...
          <p className={`mt-5 text-base ${m.muted} max-w-[560px]`}>We help construction, solar, roofing, electrical, and landscaping businesses across Africa win more work. It&apos;s a small team doing serious work for real businesses — and we&apos;re growing.</p>
          <div className="mt-7 flex flex-wrap gap-3">
            <a href="#roles" className="px-6 py-3 rounded-full bg-[#D4FF4F] text-black font-semibold hover:bg-[#c8f040]">See open roles</a>
            <a href="#life" className={`px-6 py-3 ${m.ghostBtn}`}>What it&apos;s like</a>
          </div>
        </div>
      </section>

      <section id="life" className={`py-12 ${m.sectionBand}`}>
      ...
      <section id="roles" className="py-14">
        <div className="mx-auto max-w-[1100px] px-5">
          <div className="flex items-end justify-between gap-4 flex-wrap">
            ...
            <p className={`text-[13px] ${m.faint}`}>Sample openings — update as you hire.</p>
          </div>
          <div className={`mt-7 rounded-2xl border ${m.border} divide-y divide-white/10`}>
            {ROLES.map(({ Icon, title, team, location, type }) => (
              <a key={title} href="mailto:careers@segmiq.com" className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5 hover:bg-white/[0.04] transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className={`text-[13px] ${m.faint}`}>{team}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-xs border ${m.border} rounded-full px-2.5 py-1 inline-flex items-center gap-1 ${m.muted}`}><MapPin className="w-[13px] h-[13px]" /> {location}</span>
                  <span className={`text-xs border ${m.border} rounded-full px-2.5 py-1 ${m.muted}`}>{type}</span>
                  <ArrowRight className="w-[18px] h-[18px] text-[#D4FF4F]" />
                </div>
              </a>
            ))}
          </div>
          <div className={`mt-6 ${m.elevated} p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4`}>
            <div>
              <div className="font-semibold">Don&apos;t see your role?</div>
              <p className={`text-sm ${m.muted} mt-0.5`}>If you&apos;re great at what you do and care about this market, get in touch anyway.</p>
            </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-9">
            {HIRE.map(([t, d], i) => (
              <div key={t} className={`${m.card} p-6`}>
                <div className="text-[28px] font-extrabold leading-none text-[#D4FF4F]">{i + 1}</div>
