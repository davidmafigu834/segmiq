/**
 * Per-post OpenGraph image at /blog/<slug>/opengraph-image.
 */
    ...
    (
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div style={{ color: "#fff", fontSize: 26, fontWeight: 700 }}>Segmiq</div>
        </div>

        <div style={{ display: "flex", flexDirection: "column" }}>
          <div style={{ color: "#D4FF4F", fontSize: 22, fontWeight: 700, letterSpacing: 2, marginBottom: 18 }}>{category}</div>
          <div style={{ color: "#fff", fontSize: 56, fontWeight: 800, lineHeight: 1.12, maxWidth: 960 }}>{title}</div>
        </div>

        <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 24 }}>segmiq.com/blog</div>
      </div>
    ),
    { ...size }
  );
