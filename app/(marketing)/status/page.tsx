  ...
  return (
    <>
      ...
      <section className="pb-12">
        <div className="mx-auto max-w-[1100px] px-5 max-w-[880px]">
          <div className="flex items-center justify-between">
            ...
            <div className={`hidden sm:flex items-center gap-3 text-[12px] ${m.faint}`}>
          ...
          <div className={`mt-5 rounded-2xl border ${m.border} divide-y divide-white/10`}>
            {s.components.map((c) => {
              ...
              return (
                <div key={c.key} className="p-5">
                  ...
                  <div className={`mt-2 flex items-center justify-between text-[11px] ${m.faint}`}>
        ...
        <div className="mx-auto max-w-[1100px] px-5 max-w-[880px]">
          ...
          <div className="mt-5 space-y-4">
            {activeIncidents.length === 0 && s.incidents.length === 0 && (
              <div className={`rounded-2xl border ${m.border} bg-white/[0.03] p-5`}>
                ...
                <div className={`text-[12px] ${m.faint} mt-0.5 ml-[18px]`}>Today</div>
                <p className={`text-sm ${m.muted} mt-2 ml-[18px]`}>All components are operating normally.</p>
              </div>
            )}
            {s.incidents.map((it) => (
              <div key={it.id} className={`rounded-2xl border ${m.border} bg-white/[0.03] p-5`}>
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-white/30" />
                  <div className="font-semibold">{it.title}</div>
                </div>
                <div className={`text-[12px] ${m.faint} mt-0.5 ml-[18px]`}>
                  {fmt(it.startedAt)}{it.resolvedAt ? " · resolved" : " · ongoing"}
                </div>
                <p className={`text-sm ${m.muted} mt-2 ml-[18px]`}>{it.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="pb-20">
        <div className="mx-auto max-w-[1100px] px-5 max-w-[880px]">
          <div className={`${m.elevated} p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4`}>
            <div className="flex items-center gap-3">
              <span className="grid place-items-center w-10 h-10 rounded-lg bg-[#0C0C0C] text-[#D4FF4F]"><Bell className="w-5 h-5" /></span>
              <div>
                <div className="font-semibold">Get status updates</div>
                <p className={`text-[13px] ${m.muted}`}>Be notified if a component goes down or comes back.</p>
              </div>
            </div>
