...
export async function PATCH(req: Request, { params }: { params: { clientId: string; campaignId: string } }) {
...
export async function DELETE(_req: Request, { params }: { params: { clientId: string; campaignId: string } }) {
  const session = await getServerSession(authOptions);
  if (!session?.userId) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!canAccessClient(session.role, session.clientId, params.clientId)) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  ...
  const result = await deleteAcquisitionCampaign({
    clientId: params.clientId,
