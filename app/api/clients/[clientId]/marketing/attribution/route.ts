...
export async function GET(req: Request, { params }: { params: { clientId: string } }) {
  ...
  const filters = parseMarketingFilters(url.searchParams);
  const dashboard = await getMarketingDashboard({ clientId: params.clientId, filters });
  const { exportRows: _omit, ...rest } = dashboard;
  void _omit;
