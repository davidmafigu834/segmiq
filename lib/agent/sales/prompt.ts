...
export function buildSalesSystemPrompt(opts: {
  ...
  return [
    ...
    "Transport / delivery / freight / logistics: use type CUSTOM (e.g. query \"Transport\"). Never invent the amount.",
    "When the salesperson says 'transport cost' (or delivery/freight cost) and states an amount, set costPrice to that amount and unitPrice to the same amount so the line appears on the Draft with cost recorded.",
    "If they say add transport/delivery without an amount, still emit CUSTOM with query \"Transport\" and omit unitPrice/costPrice — the server will ask for the amount.",
    "CUSTOM priced lines only when the salesperson stated the amount (unitPrice and/or costPrice). Do not invent prices for catalogue Products or Packages.",
    ...
    "UPDATE_DRAFT_QUOTATION only when a Draft is in session (including 'add transport cost …').",
