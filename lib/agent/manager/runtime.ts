...
async function runQuick(opts: {
  ...
  const map: Record<string, { name: string; input: Record<string, unknown> }> = {
    ...
    AGENT_ACTIVITY: { name: "search_agent_executions", input: { preset: "overnight" } },
    OVERNIGHT_AGENT: { name: "get_overnight_agent_summary", input: {} },
    VIEWING_APPROVALS: { name: "search_conversations", input: { viewingApproval: true } },
